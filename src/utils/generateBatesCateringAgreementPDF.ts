...
const loadImage = (src: string) =>
  new Promise<HTMLImageElement>((res, rej) => { const img = new Image(); img.crossOrigin = "anonymous";
    img.onload = () => res(img); img.onerror = rej; img.src = src; });

/* types */
export interface BatesPDFOptions {
  fullName: string;
  total: number;                 // add-ons grand total (incl. taxes/fees)
  deposit: number;               // 0 if pay-in-full
  guestCount: number;
  weddingDate: string;
...
const generateBatesCateringAgreementPDF = async ({
  fullName, total, deposit, guestCount, weddingDate, signatureImageUrl,
  paymentSummary, lineItems = [], menuSelections = {},
  ...
  doc.setFont("helvetica", "normal"); doc.setFontSize(16); doc.setTextColor(0);
  doc.text("Bates Catering Agreement & Receipt (Add-Ons)", 105, 75, { align: "center" });
  ...
  // selections / included
  if (lineItems.length || menuSelections.appetizers?.length || menuSelections.mains?.length || menuSelections.sides?.length) {
    ...
    for (const item of lineItems) {
      for (const ln of doc.splitTextToSize(`• ${item}`, 170)) { y = ensureSpace(doc, y, LINE_GAP); doc.text(ln, MARGIN_X + 5, y); y += LINE_GAP; }
    }
    const section = (label: string, items?: string[]) => {
      if (!items?.length) return; y = ensureSpace(doc, y, LINE_GAP);
      doc.text(`${label}:`, MARGIN_X + 5, y); y += LINE_GAP;
      for (const it of items) {
        for (const ln of doc.splitTextToSize(`- ${it}`, 165)) { y = ensureSpace(doc, y, LINE_GAP); doc.text(ln, MARGIN_X + 10, y); y += LINE_GAP; }
      }
    };
  ...
  for (const ln of doc.splitTextToSize(paymentSummary, 170)) { y = ensureSpace(doc, y, LINE_GAP); doc.text(ln, MARGIN_X + 5, y); y += LINE_GAP; }
  if (deposit > 0 && deposit < total) {
    doc.text(`Deposit Paid Today: $${deposit.toFixed(2)}`, MARGIN_X + 5, y); y += LINE_GAP;
    if (dueByPretty) { doc.text(`Remaining balance due by: ${dueByPretty}`, MARGIN_X + 5, y); y += LINE_GAP; }
  } else {
  ...
  /* ── EXACT “Terms” (numbered) ── */
  const H = () => {
    y = ensureSpace(doc, y, LINE_GAP + PARA_GAP);
    doc.setFont("helvetica", "bold"); doc.setFontSize(13);
    doc.text("Terms", MARGIN_X, y);
    doc.setFont("helvetica", "normal"); doc.setFontSize(12); y += 6;
  };
  const OL = (n: number, text: string) => {
    for (const ln of doc.splitTextToSize(`${n}. ${text}`, 170)) {
      y = ensureSpace(doc, y, LINE_GAP);
      doc.text(ln, MARGIN_X, y); y += LINE_GAP;
    }
    y += 2;
  };

  H();
  OL(1, "Included Services. Catering included in your Bates venue package is covered by your venue booking. Only selected add-ons are billed through this agreement.");
  OL(2, `Payment. ${total > 0 ? "You agree to pay for the selected add-ons." : "No add-ons selected; no payment is due."} If you choose Deposit + Monthly, you authorize Wed&Done to automatically charge the remaining balance in monthly installments. The final payment is due ${FINAL_DUE_DAYS} days before your wedding date.`);
  OL(3, "Non-Refundable Add-Ons. Add-on purchases are non-refundable once confirmed.");
  OL(4, "Guest Count Policy. Final guest counts are due 30 days prior to the wedding. Guest count changes are only accepted via the Guest Count Scroll between 45–30 days before your wedding.");
  OL(5, "Substitutions & Availability. Menu items, brands, or ingredients may be substituted with comparable alternatives based on seasonality or supply constraints at the caterer’s discretion.");
  OL(6, "Allergies & Dietary Needs. You are responsible for communicating allergies or dietary restrictions to Wed&Done at least 30 days prior to the event. While reasonable efforts will be made, the caterer cannot guarantee an allergen-free environment.");
  OL(7, "Force Majeure. Neither party is liable for delays or failure to perform due to events beyond reasonable control (e.g., severe weather, government action, labor disputes, supply chain issues). In such cases, services may be rescheduled or modified in good faith.");
