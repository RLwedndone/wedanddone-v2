...
const AccountScreen: React.FC<AccountScreenProps> = ({ onClose }) => {
  ...
  const [profileImage, setProfileImage] = useState("/assets/images/profile_placeholder.png");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setProfileImage(data.profileImage || "/assets/images/profile_placeholder.png");
...
const todayYMD = new Date().toISOString().slice(0, 10);
  ...
  useEffect(() => {
    ...
    function lockBanner(reasons: GuestLockReason[]) {
      ...
      const pretty: Record<GuestLockReason, string> = {
        ...
        final_submission: "final submission lock",   // 👈 add this
      };
      return `Guest count is locked due to ${reasons.map(r => pretty[r] ?? r).join(" & ")}.`;
    }
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      await saveUserProfile({
        ...
        lastName: last,
        email: u.email || "",
        profileImage: u.photoURL || "/assets/images/profile_placeholder.png",
        fianceFirst: "",
        fianceLast: "",
        phone: "",
        weddingDate: "",
      ...
      setFirstName(first);
      setLastName(last);
      setProfileImage(u.photoURL || "/assets/images/profile_placeholder.png");
      setAccountCreated(true);
      setTimeout(() => setAccountCreated(false), 3000);
    } catch (err: any) {
  ...
  if (!dateLocked && weddingDate && isPastYMD(weddingDate)) {
    ...
    try {
      ...
      if (locked && gcChanged) {
        window.dispatchEvent(new CustomEvent("openUserMenuScreen", { detail: "guestListScroll" }));
        setBannerMsg?.("Guest count is locked after a booking. Use the Guest Count Scroll to request a change.");
        return;
      }
  ...
  const onSaveGuestCount = async () => {
    await setGuestCount(gc);
    setChangesSaved(true);
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          ...
          >
            ...
            />
            <img
              src="/assets/images/camera_overlay.png"
              alt=""
        ...
        <div style={{ textAlign: "left" }}>
          ...
          <label>Email</label>
          <input value={email} onChange={(e) => setEmail(e.target.value)} style={inputStyle} disabled={!isGuest} />
          ...
          <input value={fianceFirst} onChange={(e) => setFianceFirst(e.target.value)} style={inputStyle} />

          <label>Fiancé’s Last Name</label>
          <input value={fianceLast} onChange={(e) => setFianceLast(e.target.value)} style={inputStyle} />
...
{dateLocked ? (
  <>
    <p style={{ fontSize: "1rem", marginBottom: ".5rem", color: "#333", fontWeight: "bold" }}>
      {weddingDate} — <span style={{ color: "#2c62ba" }}>{dayOfWeek}</span>
    </p>
    <p style={{ fontSize: ".9rem", fontStyle: "italic", color: "#666", marginBottom: "1.5rem" }}>
  ...
  <>
    <input
      ...
      style={{ ...inputStyle, fontFamily: "inherit", borderColor: dateError ? "#d33" : "#ccc" }}
      aria-invalid={!!dateError}
      aria-describedby={dateError ? "weddingDateError" : undefined}
    />
    {dayOfWeek && !dateError && (
      <p style={{ marginTop: "-0.5rem", marginBottom: "1rem", fontStyle: "italic", color: "#2c62ba" }}>
        That’s a <strong>{dayOfWeek}</strong>!
      </p>
    )}
    {dateError && (
      <p id="weddingDateError" style={{ color: "#d33", marginTop: "-0.25rem", marginBottom: "1rem" }}>
        {dateError}
      </p>
    )}
  </>
  ...
  {locked ? (
    <>
      <input
        ...
        value={String(gc)}
        disabled
        style={{ ...inputStyle, background: "#f4f6fb", color: "#666" }}
      />
      <div style={{ fontSize: ".9rem", color: "#666", marginTop: ".25rem" }}>
  Locked after: <strong>{(lockReasons || []).join(", ") || "a booking"}</strong>.
</div>
      <button
