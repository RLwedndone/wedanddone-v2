...
const PhotoShotList2: React.FC<PhotoShotList2Props> = ({ onNext, onBack, goToTOC }) => {
  ...
  const shotList: ShotDef[] = useMemo(() => {
    ...
    const hasSiblings = role2.brothers.length > 0 || role2.sisters.length > 0;
    ...
    if (hasFamily) {
      shots.push({ key: "whole-family", label: `${you} with Whole Family`, img: "/assets/images/Shot_Cards/LB2_shot_1_whole_family.png" });
    }
    if (hasParents) {
      shots.push({ key: "parents", label: `${you} with Parents`, img: "/assets/images/Shot_Cards/LB2_shot_2_parents.png" });
    }
    if (hasSiblings) {
      shots.push({ key: "siblings", label: `${you} with Siblings`, img: "/assets/images/Shot_Cards/LB2_shot_3_siblings.png" });
    }
    if (role2.mothers.length) {
      shots.push({ key: "mom", label: `${you} with Mom`, img: "/assets/images/Shot_Cards/LB2_shot_4_mom.png" });
    }
    if (role2.fathers.length) {
      shots.push({ key: "dad", label: `${you} with Dad`, img: "/assets/images/Shot_Cards/LB2_shot_5_dad.png" });
    }
    if (hasParty) {
      shots.push({
        key: "party",
        label: `${you} with Wedding Party`,
        img: "/assets/images/Shot_Cards/LB2_shot_6_party.png",
  ...
  const modalOptions: VIPEntry[] = currentShotDef?.allowAllVips ? allVipForParty : vip2;
  ...
  const saveModal      = () => { if (!activeShot) return; setSelection(activeShot, modalTemp); closeModal(); };
    ...
    <div className="pixie-overlay">
      <div className="pixie-card" style={{ paddingTop: "1.25rem", paddingBottom: "1.25rem" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "0.5rem" }}>
          <img src="/assets/images/ShotList2.png" alt="Shot List Icon" style={{ width: "82%", maxWidth: 150 }} />
        </div>

        <h2 style={headerFont}>
          {lb2FirstName ? `${lb2FirstName}’s Formal Family Shot List` : "Formal Family Shot List"}
        </h2>
              ...
              >
                ...
                >
                  <img
                    src={shot.img}
                    alt={shot.label}
                    style={{ width: "50%", maxWidth: 400, height: "auto", display: "block", margin: "0 auto", transition: "transform 0.2s ease" }}
                ...
                <div style={{ padding: "0.8rem 0.9rem", textAlign: "center" }}>
                  <div style={{ fontWeight: 800, marginBottom: 4 }}>{shot.label}</div>
                  {shot.helper && <div style={{ color: "#666", fontSize: "0.92rem", marginBottom: 6 }}>{shot.helper}</div>}
                  <div style={{ fontSize: "0.95rem", color: "#444", minHeight: 22 }}>
                    {skipped ? <em>Skipping this shot</em> : chosen.length ? (<><strong>Selected:</strong> {chosen.join(", ")}</>) : (<em>Click the card to choose names</em>)}
                  </div>
                </div>
              </div>
            );
        ...
        <div style={{ border: "1px solid #e7e7e7", borderRadius: 12, padding: "0.9rem", margin: "0.6rem 0 0.9rem", background: "#fafbff" }}>
          ...
          <div style={{ color: "#555", fontSize: "0.92rem", marginBottom: "0.5rem" }}>
            Tip: Use this for one‑on‑one portraits (e.g., {you} + Best Man, {you} + Sister) or any special group.
          </div>
  ...
  🪄 Back to TOC
      ...
      {activeShot && currentShotDef && (
        ...
        >
          ...
          >
            <div style={{ fontWeight: 800, fontSize: "1.15rem", marginBottom: 4, color: "#2c62ba" }}>{currentShotDef.label}</div>
            {currentShotDef.helper && <div style={{ color: "#666", marginBottom: 8 }}>{currentShotDef.helper}</div>}
