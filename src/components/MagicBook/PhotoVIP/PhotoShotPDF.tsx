...
function readSelections(key: string) {
  try {
    ...
    const sel = data?.selections && typeof data.selections === "object" ? data.selections : {};
    const out: { id: string; title: string; names: string[] }[] = [];

    (Object.entries(sel) as [string, unknown][])
      .forEach(([shotKey, names]) => {
        const list = Array.isArray(names) ? (names as unknown[]).filter(Boolean) as string[] : [];
        out.push({
          id: `${key}:${shotKey}`,
...
function buildMagicPhotoPayload(): MagicPhotoDeetsData {
  ...
  const list1 = readSelections("photoShotList1").map((s) => ({ ...s, source: "LB1" as const }));
  const list2 = readSelections("photoShotList2").map((s) => ({ ...s, source: "LB2" as const }));
  const listC = readSelections("photoShotListCombined").map((s) => ({ ...s, source: "Combined" as const }));
...
const PhotoShotPDF: React.FC<Props> = ({ onBack, onClose, goToTOC }) => {
  ...
  const handleCreatePDF = async () => {
    ...
    try {
      ...
      const fileName = `Magical-Photo-Deets-${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  return (
    <div style={{ textAlign: "center", padding: 4, color: "#111" }}>
      ...
      />
      <h2 style={{ marginBottom: 8 }}>Your Magical Photo Deets</h2>
      <p style={{ margin: "0 auto 16px", maxWidth: 560 }}>
        We’ve magically whipped up your formal shots into a clean, printable PDF. Once you hit that "Make My PDF!" button below, if you’re logged in,
        we’ll save it to your Documents. If you’re browsing as a guest, we’ll simply download it to your device. If you see anything that needs to change, just use the back button to edit your VIPs and shots.
      </p>
      ...
      >
        {!preview ? (
          <p style={{ color: "#666", margin: 0 }}>Loading your shot list…</p>
        ) : (
          (() => {
            ...
            const sections: Array<{ title: string; items: typeof preview.shots; fullName?: string }> = [
              { title: `${possessive(lb1First || "LB1")} Shots`, items: group("LB1"), fullName: lb1Full },
              { title: `${possessive(lb2First || "LB2")} Shots`, items: group("LB2"), fullName: lb2Full },
              { title: "Combined Shots", items: group("Combined") },
            ];

            const hasAny = sections.some((s) => s.items.length > 0);
            if (!hasAny) return <div style={{ color: "#666" }}>No shots selected yet.</div>;
                      ...
                      {items.map((shot: any, idx: number) => {
                        const whoList =
                          fullName && Array.isArray(shot.names)
                            ...
                            : (shot.names || []);
                        return (
                          <div key={`${title}-${shot.id || idx}`} style={{ lineHeight: 1.5, marginBottom: 6 }}>
                            <div style={{ fontWeight: 600 }}>
                              {`Shot #${idx + 1} - ${shot.title || "Untitled Shot"}`}
                            </div>
                            <div style={{ color: "#333" }}>
