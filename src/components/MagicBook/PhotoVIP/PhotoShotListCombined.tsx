...
const bucketsFromVIPs = (list: VIPEntry[]): RoleBuckets => {
  ...
  list.forEach(({ name, role }) => {
    asArray(role).forEach((raw) => {
      ...
      if (r === "mother" || r === "stepmother" || r.includes("step mother")) mothers.add(name);
      else if (r === "father" || r === "stepfather" || r.includes("step father")) fathers.add(name);
      else if (r === "brother") brothers.add(name);
      else if (r === "sister")  sisters.add(name);
      else if (
...
const PhotoShotListCombined: React.FC<PhotoShotListCombinedProps> = ({ onNext, onBack, goToTOC }) => {
  ...
  const [activeShot, setActiveShot] = useState<ShotDef["key"] | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setLb1FullName([lb1First, lb1Last].filter(Boolean).join(" ").trim());
      ...
      setLb2FullName([lb2First, lb2Last].filter(Boolean).join(" ").trim());
    } catch {}
  }, []);

  useEffect(() => {
    try {
      const list1 = JSON.parse(localStorage.getItem(VIP_LB1_KEY) || "[]");
      setVip1(Array.isArray(list1) ? (list1 as VIPEntry[]) : []);
    } catch { setVip1([]); }
    try {
      const list2 = JSON.parse(localStorage.getItem(VIP_LB2_KEY) || "[]");
  ...
  const parentsBoth = useMemo(
    () => dedup([...role1.mothers, ...role1.fathers, ...role2.mothers, ...role2.fathers]),
    [role1, role2]
  );
  const siblingsBoth = useMemo(
    () => dedup([...role1.brothers, ...role1.sisters, ...role2.brothers, ...role2.sisters]),
    [role1, role2]
  );
  const partyBoth = useMemo(() => dedup([...role1.party, ...role2.party]), [role1, role2]);

  const hasParentsCard  = parentsBoth.length >= 2;
  const hasWeddingCard  = partyBoth.length > 0;
  const hasEveryoneCard = (parentsBoth.length + siblingsBoth.length + partyBoth.length) > 0;
  ...
  useEffect(() => {
    ...
    setSelections(prev => {
      const next = { ...prev };
      Object.keys(next).forEach(k => { if (!visible.has(k as ShotDef["key"])) delete next[k]; });
      return next;
    });
  }, [shotList]);
  ...
  useEffect(() => {
    ...
    const defaults: Record<ShotDef["key"], string[]> = {
      everyone: dedup([...parentsBoth, ...siblingsBoth, ...partyBoth]),
      parents: parentsBoth,
    ...
    setSelections(prev => {
      const next = { ...prev };
      (Object.keys(defaults) as Array<ShotDef["key"]>).forEach((key) => {
        if (!visible.has(key)) return;
        if (!next[key] || next[key].length === 0) next[key] = defaults[key] || [];
      });
      return next;
    });
  }, [lb1FullName, lb2FullName, parentsBoth, siblingsBoth, partyBoth, shotList]);

  const openModal   = (key: ShotDef["key"]) => { setActiveShot(key); setModalTemp(selections[key] || []); };
  const closeModal  = () => { setActiveShot(null); setModalTemp([]); };
  ...
  const saveModal      = () => { if (!activeShot) return; setSelections(prev => ({ ...prev, [activeShot]: modalTemp })); closeModal(); };
  ...
  return (
    <div className="pixie-overlay">
      <div className="pixie-card" style={{ paddingTop: "1.25rem", paddingBottom: "1.25rem" }}>
        ...
        <p style={{ textAlign: "center", color: "#444", fontSize: "1rem", lineHeight: 1.4, margin: "0 0 1rem", padding: "0 0.5rem" }}>
          ✨ We’ve waved our little wand and pre‑filled each photo using your VIP lists. Tap a card to add or remove anyone —
          you’re the director! For one‑on‑one portraits or special groups, use the Custom Shot box below.
        </p>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ cursor: "pointer", position: "relative", display: "flex", justifyContent: "center", alignItems: "center", paddingTop: "0.25rem" }}
                >
                  <img
                    src={shot.img}
                    alt=""
                    style={{ width: "70%", maxWidth: 400, height: "auto", display: "block", margin: "0 auto", transition: "transform 0.2s ease" }}
                ...
                <div style={{ padding: "0.8rem 0.9rem", textAlign: "center" }}>
                  <div style={{ fontSize: "0.95rem", color: "#444", minHeight: 22 }}>
                    {chosen.length ? (<><strong>Selected:</strong> {chosen.join(", ")}</>) : (<em>Click the card to choose names</em>)}
                  </div>
                </div>
              </div>
            );
        ...
        <div style={{ border: "1px solid #e7e7e7", borderRadius: 12, padding: "0.9rem", margin: "0.6rem 0 0.9rem", background: "#fafbff" }}>
          <div style={{ fontWeight: 700, marginBottom: "0.5rem", color: "#2c62ba" }}>Add a Custom Shot</div>
          <div style={{ color: "#555", fontSize: "0.92rem", marginBottom: "0.5rem" }}>
            Tip: Great for one‑on‑ones (e.g., {lb1FirstName || "Partner"} + {lb2FirstName || "Partner"}) or any special grouping.
          </div>
          <CustomShotForm />
        </div>
        ...
        <div style={{ textAlign: "center", marginTop: "1rem" }}>
          <button
            onClick={onNext}
            style={{ backgroundColor: "#2c62ba", color: "#fff", fontSize: "1.05rem", padding: "0.7rem 2rem", borderRadius: 999, border: "none", cursor: "pointer", marginBottom: "0.75rem" }}
          >
            Turn the Page →
          </button>
          <br />
          <button onClick={onBack} className="boutique-back-btn" style={{ width: 250, padding: "0.75rem 1rem" }}>
            ⬅ Previous Page
          </button>
          {/* ✅ Purple Back to TOC */}
      ...
      {activeShot && (
        ...
        >
          <div
            ...
            style={{ width: "min(720px, 92vw)", background: "#fff", borderRadius: 14, boxShadow: "0 12px 28px rgba(0,0,0,0.25)", padding: "1rem 1rem 1.1rem" }}
          >
            <div style={{ color: "#666", marginBottom: 8 }}>Select everyone to include in this photo:</div>

            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.6rem" }}>
              <button onClick={selectAllModal} style={{ padding: "0.45rem 0.9rem", borderRadius: 8, border: "1px solid #ddd", cursor: "pointer" }}>Select All</button>
              <button onClick={clearAllModal}  style={{ padding: "0.45rem 0.9rem", borderRadius: 8, border: "1px solid #ddd", cursor: "pointer" }}>Clear All</button>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "0.6rem 0.9rem", maxHeight: "50vh", overflow: "auto", paddingRight: 4 }}>
              {allVIPs.map((vip) => {
                ...
                return (
                  <label key={vip.name} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <input type="checkbox" checked={checked} onChange={(e) => toggleModalName(vip.name, e.target.checked)} style={{ marginTop: 2 }} />
                    <span>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.5rem", marginTop: "0.9rem" }}>
              <button onClick={closeModal} style={{ padding: "0.55rem 1.1rem", borderRadius: 10, border: "1px solid #ddd", background: "#fff", cursor: "pointer" }}>Cancel</button>
              <button onClick={saveModal}  style={{ padding: "0.55rem 1.1rem", borderRadius: 10, border: "none", background: "#2c62ba", color: "#fff", cursor: "pointer" }}>Save</button>
            </div>
          </div>
        </div>
      )}
...
const CustomShotForm: React.FC = () => {
  ...
  const parseNames = (raw: string) => raw.split(",").map((n) => n.trim()).filter(Boolean);
  ...
  return (
    <>
      ...
      <input
        ...
        style={{ width: "100%", padding: "0.55rem 0.7rem", borderRadius: 8, border: "1px solid #dcdcdc", marginBottom: "0.6rem", fontSize: "0.95rem" }}
      />
      ...
      {error && <div style={{ color: "#c0392b", fontSize: "0.9rem", marginBottom: "0.4rem" }}>{error}</div>}

      <div style={{ textAlign: "right", marginBottom: "0.5rem" }}>
        <button
          onClick={add}
          style={{ backgroundColor: "#2c62ba", color: "#fff", fontSize: "0.95rem", padding: "0.55rem 1.1rem", borderRadius: 999, border: "none", cursor: "pointer" }}
        >
          + Add Custom Shot
        </button>
      </div>
      ...
      {customs.length > 0 && (
        <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
          {customs.map((c) => (
            <li key={c.id} style={{ border: "1px solid #eee", borderRadius: 10, padding: "0.75rem", marginBottom: "0.6rem", background: "#fff" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                <div style={{ fontWeight: 600 }}>{`Custom – ${c.label}`}</div>
                <button onClick={() => remove(c.id)} style={{ background: "none", border: "none", color: "#2c62ba", textDecoration: "underline", cursor: "pointer" }}>
                  Remove
                </button>
              </div>
