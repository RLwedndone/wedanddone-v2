...
const YumCartDessert: React.FC<YumCartDessertProps> = ({
  ...
  const [cupcakeEachByFlavor, setCupcakeEachByFlavor] = useState<Record<string, number>>(() => {
    try {
      return JSON.parse(localStorage.getItem("nvCupcakeEachByFlavor") || "{}");
    } catch {
      return {};
    }
  });
  ...
  const [goodieDozens, setGoodieDozens] = useState<Record<string, number>>(() => {
    try {
      return JSON.parse(localStorage.getItem("nvGoodieDozens") || "{}");
    } catch {
      return {};
    }
  });
  ...
  // Persist mirrors
  useEffect(() => {
    localStorage.setItem("nvCupcakeEachByFlavor", JSON.stringify(cupcakeEachByFlavor || {}));
  }, [cupcakeEachByFlavor]);
  useEffect(() => {
    localStorage.setItem("nvGoodieDozens", JSON.stringify(goodieDozens || {}));
  }, [goodieDozens]);
  ...
  useEffect(() => {
    ...
    // keep feeding a concise summary string for downstream use (not shown in cart)
    setPaymentSummaryText(
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      localStorage.setItem("yumRemainingBalance", String(round2(grandTotal - deposit25)));
...
return (
  <div className="pixie-card pixie-card--modal" style={{ maxWidth: 700 }}>
    ...
    <div className="pixie-card__body" style={{ textAlign: "center" }}>
      ...
      {/* Quantities – Cupcakes (aligned grid) */}
          ...
          {cupcakes.map((title) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="px-qty"
                  style={{ display: "flex", alignItems: "center", gap: "0.5rem", width: 220 }}
                >
                  <button
                    type="button"
                  ...
                  <input
                    ...
                    value={each}
                    onChange={(e) =>
                      setEach(parseInt(e.target.value || "0", 10) || CUPCAKE_MIN_EACH)
                    }
                    className="px-input-number"
                    style={{ width: 90, textAlign: "center", boxSizing: "border-box" }}
                  />
      ...
      {/* Quantities – Goodies (aligned grid) */}
          ...
          {goodies.map((key) => {
            ...
            return (
              <div
                ...
                style={{
                  display: "grid",
                  // Shrinks the first column as needed so the row never overflows
              ...
              >
                ...
                {/* Dozens label (fixed width column keeps alignment) */}
                <div style={{ fontSize: ".95rem", textAlign: "right", whiteSpace: "nowrap" }}>Dozens:</div>
            
                {/* Qty controls (fixed width so it never pushes the grid) */}
                <div
                  className="px-qty"
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "0.5rem",
                    width: 180,                // ⬅️ narrower, predictable
                    maxWidth: 180,
                    justifyContent: "center",
                  }}
                >
                  <button
                    ...
                    disabled={dz <= min}
                    style={{ width: 32, height: 32, flex: "0 0 32px" }}  // ⬅️ 32px buttons
                  >
                    <img src="/assets/icons/qty_minus_pink_glossy.svg" alt="" aria-hidden="true" />
                  </button>
                ...
                {/* Price (never wraps, so width is predictable) */}
