...
interface VicVerradoCheckOutProps {
  ...
  guestCount: number;
  lineItems: string[];
  flowerTier?: VVFlowerTier; // optional (we’ll also read from LS)
  menuSelections: {
    hors?: string[];
    salads?: string[];
    starches?: string[];
...
const VicVerradoCheckOutCatering: React.FC<VicVerradoCheckOutProps> = ({
  ...
  const handleSuccess = async ({ customerId }: { customerId?: string } = {}) => {
    ...
    try {
      ...
      onComplete(); // overlay advances to Catering TY
      ...
      const pdfBlob = await generateVicVerradoAgreementPDF({
        ...
        selections: {
          ...
          starches:
            menuSelections.starches || vvSelections.starches || vvSelections.starch || [],
          vegetables:
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: "1.25rem", fontSize: "1rem", textAlign: "center" }}>
      ...
      <div style={{ marginTop: "2rem", display: "flex", justifyContent: "center" }}>
        <button
          ...
          style={{ width: "250px", padding: "0.75rem 1rem", fontSize: "1rem" }}
          onClick={onBack}
        >
          ⬅ Back
        </button>
