// src/components/NewYumBuild/CustomVenues/VicandVerrado/VicVerradoDessertCart.tsx
...
const VicVerradoDessertCart: React.FC<Props> = ({
  dessertStyle,
  flavorFilling,
  cakeStyle,
  treatType = "",
  ...
  return (
    <div className="pixie-overlay">
      <div className="pixie-card" style={{ maxWidth: "700px", textAlign: "center" }}>
        ...
        <h2 style={{ fontFamily: "'Jenna Sue', cursive", fontSize: "2.2rem", color: "#2c62ba" }}>
          Your Dessert Order
        </h2>
        ...
        <div style={{ marginBottom: "1.25rem" }}>
          <div style={{ fontWeight: 700, marginBottom: ".35rem" }}>Guest Count</div>
          ...
          {/* 💲 Per-serving cost line (only for tiered cake) */}
          {dessertStyle === "tieredCake" && gc > 0 && (
            <div style={{ marginTop: ".4rem", fontSize: ".95rem", color: "#444" }}>
        ...
        <div style={{ marginBottom: "2rem" }}>
          <h3 style={{ fontFamily: "'Jenna Sue', cursive", fontSize: "1.6rem", color: "#2c62ba" }}>
            Style:
          </h3>
          <p>{formattedDessertStyle}</p>

          {/* ── Small Cake details (or generic for tiered-only) ── */}
          {dessertStyle !== "treatsOnly" && (
            <>
              ...
              <p>{flavorFilling.length > 0 ? flavorFilling.join(" + ") : "Not selected"}</p>
              ...
              {/* 🍰 Only for Small Cake + Treats: show the cutting cake line */}
          ...
          {(dessertStyle === "smallCakeTreats" || dessertStyle === "treatsOnly") && (
            <>
              ...
              <p>{treatType ? (treatType === "cupcakes" ? "Cupcakes" : "Goodies") : "Not selected"}</p>

              {treatType === "cupcakes" && (
                <>
                  <h4 style={{ marginTop: ".5rem" }}>Cupcake Flavors:</h4>
                  <p>{(cupcakes || []).map((t) => t.split("–")[0].trim()).join(", ")}</p>
                </>
              )}

              {treatType === "goodies" && (goodies?.length ?? 0) > 0 && (
                <>
                  <h4 style={{ marginTop: ".5rem" }}>Goodies:</h4>
                  <p>{goodies.map(goodieLabel).join(", ")}</p>
                </>
              )}
            </>
          )}
            ...
            {cupcakes.map((title) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: ".9rem", color: "#444" }}>
                      Quantity (each):
                      <input
                        type="number"
                        ...
                        onChange={(e) => {
                          ...
                          setCupcakeEachByFlavor((prev) => ({ ...prev, [title]: nextVal }));
                        }}
                        style={{
                          display: "block",
                          marginTop: ".25rem",
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: ".9rem", color: "#444" }}>
                      ...
                      <input
                        ...
                        value={dz}
                        onChange={(e) => {
                          const next = Math.max(min, parseInt(e.target.value || "0", 10) || 0);
        ...
        <div style={{ marginTop: ".25rem" }}>
          ...
          <div style={{ marginBottom: ".25rem", color: "#444" }}>
            Taxes & fees: ${Number(taxesAndFees).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}
          </div>
          <div style={{ marginBottom: ".75rem", fontWeight: 800 }}>
            Total: ${Number(grandTotal).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}
          </div>
        </div>

        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
          <button className="boutique-primary-btn" onClick={handleContinue} style={{ width: "250px" }}>
            Confirm & Book
          </button>
          <button className="boutique-back-btn" onClick={onStartOver} style={{ width: "250px" }}>
            ⬅ Back to Menu
          </button>
        </div>
      </div>
...
export default VicVerradoDessertCart;