...
interface TubacDessertContractProps {
  total: number;
  guestCount: number;
  ...
  setStep: (step: any) => void; // overlay type will be wired by parent
  onClose: () => void;
...
const TubacDessertContract: React.FC<TubacDessertContractProps> = ({
  total,
  guestCount,
  weddingDate,
  dayOfWeek,
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // ✅ hydrate wedding date + weekday from FS (with LS fallback)
        const ymdFromFS =
        ...
        console.error("🔥 [Tubac][DessertContract] boot error:", err);
      }
    });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("🔥 [Tubac][DessertContract] mirror error:", err);
      }
    })();
...
export default TubacDessertContract;