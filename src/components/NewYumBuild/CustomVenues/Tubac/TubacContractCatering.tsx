...
interface TubacContractProps {
  ...
  onBack: () => void;
  onContinueToCheckout: () => void;   // ✅ make sure this is here
  onComplete: () => void;
  onClose: () => void;                 // ✅ you destructure this, so include it
}

const TubacContractCatering: React.FC<TubacContractProps> = ({
  total,
  guestCount,
  weddingDate,
  dayOfWeek,
  ...
  onBack,
  onContinueToCheckout,               // ✅ don’t forget to destructure it
  onComplete,
  onClose,
}) => {
  const auth = getAuth();
  ...
  // Merge the new fields; fall back to old 'hors' or 'appetizers' if needed
const horsList: string[] =
(menuSelections.horsPassed?.length || menuSelections.horsDisplayed?.length)
  ? [...(menuSelections.horsPassed || []), ...(menuSelections.horsDisplayed || [])]
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("tubacLineItems", JSON.stringify(lineItems || []));
      localStorage.setItem("tubacMenuSelections", JSON.stringify(menuSelections || {}));
    } catch {}
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // mirror to subdocs
        await setDoc(doc(userRef, "yumYumData", "tubacMenuSelections"), { menuSelections }, { merge: true });
        await setDoc(
          doc(userRef, "yumYumData", "tubacMeta"),
          { serviceOption, selectedTier, lineItems, total, savedAt: new Date().toISOString() },
          { merge: true }
        );
      } catch (err) {
    ...
    return () => unsubscribe();
  }, [lineItems, menuSelections, serviceOption, selectedTier, total]);
  ...
  const handleSuccess = async () => {
    ...
    try {
      ...
      const cuisineLabel = `Tubac Catering — ${serviceOption === "plated" ? "Plated" : "Buffet"} (${selectedTier})`;
      ...
      const pdfBlob = await generateYumAgreementPDF({
        ...
        menuSelections: {
          appetizers: horsList,                       // ✅ use the combined list
          mains: menuSelections.entrees || [],
          sides: menuSelections.sides || [],
        },
      });

      await uploadPdfBlob(pdfBlob, userId, "TubacCateringAgreement");
      ...
      console.error("❌ Tubac contract error:", err);
    } finally {
    ...
    <div className="pixie-card" style={{ maxWidth: 700 }}>
      ...
      <div className="pixie-card__body" style={{ textAlign: "center" }}>
        ...
        <p className="px-prose-narrow" style={{ marginBottom: 8 }}>
          You’re booking <strong>Tubac {serviceOption === "plated" ? "Plated Dinner" : "Buffet"}</strong> — <strong>{selectedTier}</strong> for{" "}
        ...
        {/* Booking Terms — standardized list */}
  ...
  onClick={() => {
    ...
    // ✅ only this — don't call onComplete here, and don't call setStep here
...
<button
  ...
  onClick={onBack}                 // ✅ was: () => setStep("cateringCart")
  style={{ width: 250 }}
>
  ← Back to Cart
</button>
      ...
      {/* Optional hidden action to call after Stripe success */}
      {isGenerating ? (
...
export default TubacContractCatering;