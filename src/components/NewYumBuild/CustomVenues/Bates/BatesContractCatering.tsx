...
interface BatesContractCateringProps {
  ...
  addonsTotal: number;           // raw add-ons subtotal (pre tax+fees) – kept for record only
...
/** 👉 new: final due date for monthly plan **/
...
const BatesContractCatering: React.FC<BatesContractCateringProps> = ({
  ...
  const [payFull, setPayFull] = useState(true); // full vs deposit+monthly
...
const [weekdayPretty, setWeekdayPretty] = useState<string | null>(dayOfWeek || null);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("batesMenuSelections", JSON.stringify(menuSelections || {}));
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // ✅ NEW: hydrate date from Firestore (and LS fallback), then weekday
        const ymdFromFS =
          data?.weddingDate ||
          data?.wedding?.date ||
        ...
        await setDoc(doc(userRef, "yumYumData", "batesMenuSelections"), menuSelections, { merge: true });
        await setDoc(doc(userRef, "yumYumData", "batesLineItems"), { lineItems }, { merge: true });
      } catch (e) {
  ...
  ? new Date(`${weddingYMD}T12:00:00`).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
  : "your wedding date";
  ...
  const handleSignatureSubmit = async () => {
    ...
    if (useTextSignature && typedSignature.trim()) {
      ...
      try {
        ...
        alert("Something went wrong when capturing your signature. Please try again.");
        return;
      }
    } else {
...
await setDoc(
  doc(db, "users", userId),
  {
    progress: { yumYum: { step: "cateringCheckout" } }, // 👈 consistent with overlay
    yumYumBillingPlan: billingRobot,
    // no purchases here; let checkout success add it
  },
  { merge: true }
);
...
localStorage.setItem("yumFinalDueDays", String(FINAL_DUE_DAYS));
localStorage.setItem("yumDepositAmount", String(payFull ? total : depositAmount));
localStorage.setItem("yumTotalDue", String(total));
localStorage.setItem("yumRemainingBalance", String(remainingBalance));
localStorage.setItem("yumBillingRobot", JSON.stringify(billingRobot));
  ...
  return (
    <div className="pixie-card pixie-card--modal" style={{ maxWidth: 720 }}>
      ...
      <div className="pixie-card__body" style={{ textAlign: "center" }}>
        ...
        <div className="px-prose-narrow" style={{ margin: "8px auto 12px", maxWidth: 620 }}>
          <p><strong>Good news!</strong> Your Bates venue booking already includes your catering.</p>
          <p style={{ marginTop: 6 }}>
            This agreement confirms your menu selections and any <em>optional add-ons</em> you chose below.
            Final guest counts are due <strong>30 days</strong> prior to the wedding.
          </p>
          <p style={{ marginTop: 6 }}>
            Guest count increases are handled via the <strong>Guest Count Scroll</strong> between <strong>45–30 days</strong> pre-wedding.
          </p>
        </div>
        ...
        {/* Terms */}
        <div className="px-section" style={{ textAlign: "left", margin: "0 auto 14px", maxWidth: 640 }}>
          <h3 className="px-title" style={{ textAlign: "center", marginBottom: 6 }}>Terms</h3>
          <ol className="px-prose-narrow" style={{ margin: 0, paddingLeft: "1.25rem", lineHeight: 1.6 }}>
            <li><strong>Included Services.</strong> Catering included in your Bates venue package is covered by your venue booking. Only selected <em>add-ons</em> are billed through this agreement.</li>
            <li><strong>Payment.</strong> {total > 0 ? "You agree to pay for the selected add-ons." : "No add-ons selected; no payment is due."} If you choose <em>Deposit + Monthly</em>, you authorize Wed&Done to charge monthly; the final payment is due <strong>{FINAL_DUE_DAYS} days</strong> before your wedding.</li>
            <li><strong>Non-Refundable Add-Ons.</strong> Add-on purchases are non-refundable once confirmed.</li>
            <li><strong>Guest Count Policy.</strong> Final guest counts are due <strong>35 days</strong> prior. </li>
            <li><strong>Substitutions & Availability.</strong> Items may be substituted with comparable alternatives due to seasonality or supply.</li>
            <li><strong>Allergies & Dietary Needs.</strong> Communicate restrictions at least 30 days prior. Allergen-free outcomes aren’t guaranteed.</li>
            <li><strong>Force Majeure.</strong> Neither party is liable for events beyond reasonable control; services may be rescheduled or modified in good faith.</li>
          </ol>
        </div>
        ...
        {total > 0 && (
          <>
            <h4 className="px-title" style={{ marginTop: 6, marginBottom: 6 }}>Choose how you’d like to pay:</h4>
