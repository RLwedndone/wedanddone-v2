// src/components/NewYumBuild/CustomVenues/Bates/BatesDessertCart.tsx
...
const BatesDessertCart: React.FC<Props> = ({
  dessertStyle,
  flavorFilling,
  cakeStyle,
  treatType = "",
  ...
  >(() => {
    try {
      return JSON.parse(
        localStorage.getItem("yumNvCupcakeEachByFlavor") || "{}"
      );
    } catch {
      return {};
    }
  ...
  >(() => {
    try {
      return JSON.parse(
        localStorage.getItem("yumNvGoodieDozens") || "{}"
      );
    } catch {
      return {};
    }
  ...
  // 1. Dessert subtotal BEFORE delivery
  ...
  // 2. Add flat delivery fee for Bates
  const subtotalWithDelivery = useMemo(
    () => round2(dessertSubtotal + DELIVERY_FEE),
    [dessertSubtotal]
  );

  // 3. Taxes & Stripe fee on (dessert + delivery)
  ...
  useEffect(() => {
    // tell parent/checkout screens the FINAL with delivery+tax+fees
    ...
    // Delivery fee line item for Bates
    if (DELIVERY_FEE > 0) {
      items.push(
        `Delivery Fee — $${DELIVERY_FEE} flat`
      );
    }

    setLineItems(items);

    // concise summary string for downstream use (email/PDF/etc)
    setPaymentSummaryText(
  ...
  const handleContinue = async () => {
    ...
    try {
      // final total INCLUDING delivery + tax + stripe
      localStorage.setItem(
        "yumTotal",
        String(grandTotalFinal)
      );

      const depositLocal = round2(
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: 24 }}>
              <h3 className="px-title" style={{ marginBottom: 4 }}>
                Style:
              </h3>
            ...
            {treatType === "goodies" && goodies.length > 0 && (
              ...
              >
                ...
                >
                  Auto-filled for your guest count (≈ 1 piece per guest). Edit as you like.
                  Minimums per flavor apply.
                </div>
                    ...
                    >
                      ...
                      >
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setDz(
                              parseInt(e.target.value || "0", 10) || min
                            )
                          }
                          className="px-input-number"
                          style={{
...
export default BatesDessertCart;