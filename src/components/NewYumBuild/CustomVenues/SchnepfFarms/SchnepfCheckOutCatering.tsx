...
if (typeof window !== "undefined") {
  window.addEventListener("error", (e) =>
    console.error("[SCH][Global] window.onerror:", (e as any)?.error || e?.message || e)
  );
...
const SchnepfCheckOutCatering: React.FC<Props> = ({
  ...
  const resolvedPlan = planFromPayload || (rawPlan === "deposit" ? "deposit" : "full");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (user) {
        ...
        try {
          const snapC = await getDoc(doc(db, "users", user.uid, "yumYumData", "checkout"));
          if (snapC.exists()) {
  ...
  const handleSuccess = async ({ customerId }: { customerId?: string } = {}) => {
    console.group("%c[SCH][Checkout] onSuccess", "color:#22c55e;font-weight:700");
    ...
    const breaker = setTimeout(() => {
      console.warn("[SCH][Checkout] breaker: clearing local spinner after 20s");
    ...
    try {
      ...
      console.log("[SCH][Checkout] userDoc snapshot:", userDoc?.firstName, userDoc?.lastName);
      ...
      if (customerId && customerId !== userDoc?.stripeCustomerId) {
        ...
        await updateDoc(userRef, { stripeCustomerId: customerId, "stripe.updatedAt": serverTimestamp() });
      ...
      console.log("[SCH][Checkout] Routing to CateringThankYou now via onComplete()");
      onComplete();
      ...
      if (!guestCountFinal) {
        try {
          const { getGuestState } = await import("../../../../utils/guestCountStore");
      ...
      try {
        const pdfBlob = await generateSchnepfAgreementPDF({
          fullName: `${userDoc?.firstName || firstName || "Magic"} ${userDoc?.lastName || lastName || "User"}`,
          ...
          paymentSummary: isDeposit
            ? `Deposit today: $${amountDueToday.toFixed(2)}. Remaining $${remainingBal.toFixed(
                2
              )} due by ${prettyDue}.`
      ...
      try {
        ...
        await updateDoc(userRef, {
          ...
          purchases: arrayUnion({
            ...
            contractTotal: Number(finalTotal.toFixed(2)),
            payFull: !isDeposit,
            deposit: isDeposit ? amountNow : Number(finalTotal.toFixed(2)),
            method: isDeposit ? "deposit" : "full",
  ...
  if (isGenerating) {
    ...
    return (
      <div className="pixie-card pixie-card--modal" style={{ maxWidth: 420, position: "relative" }}>
        <div className="pixie-card__body" style={{ textAlign: "center" }}>
          ...
          />
          <p style={{ fontSize: "1.05rem", color: "#2c62ba", fontStyle: "italic", margin: 0 }}>
  ...
  const summaryText = isDeposit
    ? `Deposit today: $${amountDueToday.toFixed(2)} (25%). Remaining $${remainingBalance.toFixed(
        2
