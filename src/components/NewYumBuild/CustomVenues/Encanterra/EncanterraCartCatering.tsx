...
const SALES_TAX_RATE = 0.0895;
const STRIPE_RATE = 0.029;
const STRIPE_FLAT_FEE = 0.3;
const SERVICE_FEE_RATE = 0.22; // 22% Encanterra service fee
...
const EncanterraCartCatering: React.FC<Props> = ({
  ...
  // 22% service fee on food & beverage subtotal
  const serviceFee = subtotal * SERVICE_FEE_RATE;

  // Tax on the food subtotal
  const taxes = subtotal * SALES_TAX_RATE;

  // Stripe fees on the charge
  const cardFees =
    subtotal > 0 ? subtotal * STRIPE_RATE + STRIPE_FLAT_FEE : 0;

  // Grand total the user pays today
  const grandTotal = subtotal + serviceFee + taxes + cardFees;
  ...
  const limitsByTier: Record<
    Tier,
    { hors: number; salads: number; entrees: number; sides: number }
  > = {
  ...
  useEffect(() => {
    ...
    setPaymentSummaryText(
      `You're paying $${grandTotal.toFixed(
        2
      )} today for your ${TIER_LABEL[selectedTier]} menu (includes 22% service fee, taxes, & card fees).`
    );
  }, [
    gc,
    perGuest,
  ...
  const handleContinue = async () => {
    ...
    const st = await getGuestState();
    const value = Number(
      st.value ?? localStorage.getItem("encanterraGuestCount") ?? 0
    );
    const reasons = (
  ...
  const List: React.FC<{ title: string; items: string[] }> = ({ title, items }) => (
    <div style={{ marginBottom: "1.25rem" }}>
      ...
      {items.length ? (
        ...
        <p style={{ margin: ".25rem 0", opacity: 0.7, padding: "0 16px" }}>
          None selected
        </p>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      {showFnbProgress && (
        ...
        >
          Progress toward the ${fnbMinimumDollars.toLocaleString()} food & beverage
          minimum: {subtotal.toFixed(2)} ({Math.round(fnbProgress * 100)}%)
          <div
            style={{
              height: 8,
              width: "100%",
      ...
      >
        Includes 22% service fee, taxes &amp; card fees.
      </div>
