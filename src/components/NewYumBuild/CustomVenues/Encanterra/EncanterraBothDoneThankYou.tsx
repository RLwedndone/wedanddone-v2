// src/components/NewYumBuild/CustomVenues/Encanterra/EncanterraBothDoneThankYou.tsx
...
const EncanterraBothDoneThankYou: React.FC<Props> = ({ onClose }) => {
  // Mark progress for restore
  useEffect(() => {
    try {
      localStorage.setItem("yumStep", "encanterraBothDoneThankYou");
    } catch {}
    const user = getAuth().currentUser;
    if (user) {
      updateDoc(doc(db, "users", user.uid), {
        "progress.yumYum.step": "encanterraBothDoneThankYou",
      }).catch(() => {});
    }
  }, []);
...
export default EncanterraBothDoneThankYou;