...
interface LegalStuffProps {
  // Either of these can be used by callers – they behave the same.
  onBack?: () => void;
  onClose?: () => void;
}

const LegalStuff: React.FC<LegalStuffProps> = ({ onBack, onClose }) => {
  const handleBack = onBack || onClose || (() => {});

  return (
    <div
      className="pixie-card__body px-prose"
      style={{ paddingTop: 10, maxWidth: 720, margin: "0 auto" }}
    >
      {/* Header icon */}
      ...
      <section style={{ marginTop: 28 }}>
        ...
        <p className="px-body-sm">We use your information to:</p>
        <ul className="px-list">
      ...
      <div style={{ textAlign: "center", marginTop: "2.5rem" }}>
        <button onClick={handleBack} className="boutique-back-btn">
          ← Back
        </button>
      </div>
    </div>
