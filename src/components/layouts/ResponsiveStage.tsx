...
>
      {hotspots.map((h) => {
        ...
        const width =
          h.widthClamp ??
          (typeof h.widthPct === "number" ? `${h.widthPct}%` : "clamp(72px, 10vw, 180px)");
        ...
        return (
            ...
            key={h.id}
            onClick={h.onClick}
            className={`stage-btn ${h.className ?? ""}`}
            style={{
              position: "absolute",
              top: `${h.topPct}%`,
              left: `${h.leftPct}%`,
              ...
              // sizing: prefer widthClamp if provided, else widthPct of viewport
