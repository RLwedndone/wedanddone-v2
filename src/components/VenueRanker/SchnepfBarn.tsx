...
interface SchnepfBarnProps {
  onContinue: () => void;   // go to next venue
  onBack: () => void;       // go to previous venue
  ...
  goToExplore: () => void;  // purple "Start over" → Explore selector
}

const SchnepfBarn: React.FC<SchnepfBarnProps> = ({
  onContinue,
  onBack,
  onClose,
  ...
  const venueId = "schnepfbarn";
  const [showError, setShowError] = useState(false);
  const [showTooltip, setShowTooltip] = useState(false);

  // ✅ guard against undefined selections/rankings
...
>
  ...
  <iframe
    src="https://player.vimeo.com/video/829959049?autoplay=0&muted=0&playsinline=1"
    title="Schnepf Barn"
    loading="lazy"
